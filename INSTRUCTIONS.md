...
## 8. Design notes, in case you want to change things

Everything is driven by CSS custom properties at the top of `assets/css/main.css`.
The two accent colours are a diverging pair borrowed from CMB temperature
colormaps and they carry meaning throughout — `--cold` for anything below
expectation (undercoverage, nulls, subtracted foreground), `--warm` for anything
above it. If you swap them for something arbitrary, that logic breaks quietly.

Type is Source Serif 4 for reading and IBM Plex Sans for controls and numbers,
pulled from Google Fonts with system fallbacks. If you want the site to work
fully offline, download both and replace the `@import` at the top of the
stylesheet.

Accessibility floor is already in: keyboard focus is visible, `prefers-reduced-motion`
is respected, plots carry `aria-label` descriptions, and the whole thing works
down to a 360 px viewport.
